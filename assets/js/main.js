    ...
    window.CanvasSlideshow = function( options ) {
      ...
      this.initPixi = function() {
        ...
        function setup() {
            ...
            liquidDisplacementSprite.y = renderer.height / 2;
            liquidVx = liquidDisplacementSprite.x;
      ...
      // Notice the scale is ‘0’ for the displacement, that’s because it will be set to a height as soon as the mouse moves.
            ...
            stage.addChild(liquidDisplacementSprite);
            slidesContainer.filters = [liquidDisplacementFilter];
            liquidDisplacementFilter.scale.x = 0;
            liquidDisplacementFilter.scale.y = 0;
            // liquidBg = new PIXI.Sprite(PIXI.loader.resources["bg.jpg"].texture);
            // liquidBg.width = app.renderer.width;
        ...
        function onPointerMove(eventData) {
            liquidPosX = eventData.data.global.x;
        }

        function loop() {
            requestAnimationFrame(loop);
            liquidVx += (liquidPosX - liquidDisplacementSprite.x) * 0.045;
            liquidDisplacementSprite.x = liquidVx;
            var disp = Math.floor(liquidPosX - liquidDisplacementSprite.x);
            if (disp < 0) disp = -disp;
            var fs = map(disp, 0, 500, 0, 120);
            disp = map(disp, 0, 500, 0.1, 0.6);
            liquidDisplacementSprite.scale.x = disp;
            liquidDisplacementFilter.scale.x = fs;
        }
        ...
        if ( options.fullScreen === true ) {
          ...
          // renderer.view.style.position  = 'absolute'; //new addition
          renderer.view.style.webkitTransform = 'translate( -50%, -50% ) scale(1.5)';
          renderer.view.style.transform = 'translate( -50%, -50% ) scale(1.5)';           
        } else {
